...
`schema_migrations` records every applied database transition. Startup applies missing migrations in ascending order inside transactions, validates the required table set and refuses a migration history newer than the application. Databases created by 0.1.0 before the ledger existed are validated against the complete v1 table set and adopted without rewriting rows. Migrations are forward-only: restoring an older state uses a tested database backup, not a down-migration.
