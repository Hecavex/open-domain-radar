...
def audit_examples() -> None:
    value = json.loads(read_text(ROOT / "examples" / "targets.example.json"))
    if value.get("schemaVersion") != 1 or not isinstance(value.get("targets"), list):
        fail("invalid example target schema")
    for target in value["targets"]:
        if target.get("enabled") is not False:
            fail("example targets must be disabled")
        domains = target.get("officialDomains", [])
        if not domains or any(not str(domain).endswith(".example") for domain in domains):
            fail("example target domains must use .example")


def audit_repository_material() -> None:
    """Reject runtime state and obvious credentials before a commit is published."""

    ignored_parts = {".git", ".mypy_cache", ".pytest_cache", ".ruff_cache", "__pycache__", "htmlcov"}
    for path in ROOT.rglob("*"):
...
def main() -> int:
    checks = (
        ...
        audit_javascript,
        audit_examples,
        audit_repository_material,
    )
    for check in checks:
        check()
    print(f"Project verification passed ({len(checks)} boundary checks).")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"Project verification failed: {error}", file=sys.stderr)
