...
from .db import Database, restore_sqlite_backup, sqlite_backup
...
def build_parser() -> argparse.ArgumentParser:
    ...
    restore = commands.add_parser("restore", help="validate and restore a SQLite backup while services are stopped")
    restore.add_argument("source")
    restore.add_argument(
        "--force",
        action="store_true",
        help="replace an existing database after saving a rollback copy",
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "restore":
        return _restore_database(settings, args.source, args.force)
...
def _restore_database(settings: Settings, source: str, force: bool) -> int:
    """Restore a validated backup without starting or initializing the app."""
    try:
        result = restore_sqlite_backup(settings.database_url, Path(source), force=force)
    except (FileExistsError, ValueError) as exc:
        raise SystemExit(str(exc)) from exc
    print(f"restored schema v{result.schema_version} to {result.destination}")
    if result.rollback_backup:
        print(f"previous database saved to {result.rollback_backup}")
    return 0
