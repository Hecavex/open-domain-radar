(() => {
  ...
  const providerDefaults = {
    ...
    virustotal: { title: "VirusTotal", requires_key: true, description: "Optional reputation and relationship context from authorised API access.", config: { key: "result_limit", label: "Maximum results per pivot", min: 1, max: 40, default: 40 } },
  };
  ...
  function updateReviewActions(candidate) {
    ...
    if (["false_positive", "closed"].includes(status)) {
      ...
      help.textContent = "Choose a disposition that reflects the evidence currently available.";
    }
  }
