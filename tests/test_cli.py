...
from open_domain_radar.models import Admin, WatchTarget
...
def test_cli_restores_backup_and_requires_explicit_replace(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    live = tmp_path / "live"
    configure_environment(monkeypatch, live)
    assert main(["init"]) == 0
    settings = Settings.from_env()
    database = Database(settings)
    with database.session() as session:
        session.add(WatchTarget(name="Backed up", aliases=["backup"], official_domains=[]))
    ...
    database = Database(settings)
    with database.session() as session:
        session.add(WatchTarget(name="After backup", aliases=["after"], official_domains=[]))
    database.engine.dispose()
    with pytest.raises(SystemExit, match="--force"):
        main(["restore", str(backup)])
    assert main(["restore", str(backup), "--force"]) == 0

    database = Database(settings)
    with database.session() as session:
        names = session.scalars(select(WatchTarget.name).order_by(WatchTarget.name)).all()
    database.engine.dispose()
